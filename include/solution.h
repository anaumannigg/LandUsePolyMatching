...
public:
	...
	//returns a vector with one entry per match describing [#polys_from_set1_in_match,#polys_from_set2_in_match]
	std::vector<std::pair<int,int>> getMatchMultiplicities();

	std::pair<std::vector<std::pair<int,int>>,std::vector<std::pair<int,int>>> getMatchMultiplicitiesPerPolygon();
};
