...
std::vector<std::pair<int,int>> Solution::getMatchMultiplicities() {
	std::vector<std::pair<int,int>> multiplicities(this->match_count, {0,0});

	//matches begin with negative IDs, consider this, multplicities-array is offset
	int offset = abs(this->match_ids.first)-1;

	//iterate over all polygons
	for (int i = 0; i < this->set1_match_index.size(); i++) {
		multiplicities[this->set1_match_index[i] + offset].first++;
	}
	for (int j = 0; j < this->set2_match_index.size(); j++) {
		multiplicities[this->set2_match_index[j] + offset].second++;
	}

	return multiplicities;
}

std::pair<std::vector<std::pair<int,int>>,std::vector<std::pair<int,int>>> Solution::getMatchMultiplicitiesPerPolygon() {
	auto multiplicities = this->getMatchMultiplicities();

	std::vector<std::pair<int,int>> multiplicities_set1(this->set1_match_index.size(), {0,0});
	std::vector<std::pair<int,int>> multiplicities_set2(this->set2_match_index.size(), {0,0});

	int offset = abs(this->match_ids.first)-1;

	for (int i=0; i<this->set1_match_index.size(); i++) {
		int match_id = this->set1_match_index[i] + offset;
		multiplicities_set1[i] = multiplicities[match_id];
	}

	for (int j=0; j<this->set2_match_index.size(); j++) {
		int match_id = this->set2_match_index[j] + offset;
		//invert multiplicities, so this dataset stands on index 0, the other dataset on index 1
		multiplicities_set2[j] = {multiplicities[match_id].second, multiplicities[match_id].first};
		//multiplicities_set1[j] = multiplicities[match_id];
	}

	return {multiplicities_set1, multiplicities_set2};
}
