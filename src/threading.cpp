...
void Threading::decomposeIntoConnectedComponentsUsingTXT(DecompositionTask task,
    ...
    int batch_begin = task.interval.first;
    int batch_end = std::min(batch_begin + task.batch_size, task.interval.second + 1);  // inclusive

    // Calculate batch ID and file path
    int batch_id = batch_begin / task.batch_size;
    std::string binary_path = "../input/" + task.data_name + "/tmp/data_batch" + std::to_string(batch_id) + ".bin";
    ...
    // Skip lines until reaching batch_begin
    std::string line;
    int current_line = 0;
    while (current_line < batch_begin && std::getline(infile, line)) {
        current_line++;
    }

    // Now read only the lines in [batch_begin, batch_end)
    for (int set = batch_begin; set < batch_end && std::getline(infile, line); ++set) {
        try {
            std::string a_part, b_part;

            // Safely split line into a_part and b_part even if one is missing
            size_t space_pos = line.find(' ');
            if (space_pos == std::string::npos) {
                a_part = line;
                b_part = "";
            } else {
                a_part = line.substr(0, space_pos);
                b_part = line.substr(space_pos + 1);
            }

            std::vector<int> group_indices1, group_indices2;
            std::stringstream ss_a(a_part), ss_b(b_part);
            std::string item;

            while (std::getline(ss_a, item, ',')) {
                if (!item.empty()) group_indices1.push_back(std::stoi(item));
            }

            while (std::getline(ss_b, item, ',')) {
                if (!item.empty()) group_indices2.push_back(std::stoi(item));
            }

            std::vector<Polygon_wh> group1, group2;
            for (int i : group_indices1) {
                if (i >= 0 && i < polys1.size()) group1.push_back(polys1[i]);
            }
            for (int j : group_indices2) {
                if (j >= 0 && j < polys2.size()) group2.push_back(polys2[j]);
            }

            writePolysToBinaryFile(group1, group2, binary_path, set);
        } catch (const std::exception& e) {
            std::cerr << "Error processing set " << set << ": " << e.what() << std::endl;
            writePolysToBinaryFile(std::vector<Polygon_wh>(), std::vector<Polygon_wh>(), binary_path, set);
        }
