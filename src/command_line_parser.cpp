...
void print_help() {
    std::cout << "Usage: polygonmatching [-d dataset_name] [-l lambda]\n"
              ...
              << "  -dd1 descriptor1     (optional, default is iacs) Specify the descriptor in the first file that is added as _dd1.gpkg.\n"
              << "  -dd2 descriptor2     (optional, default is sat)  Specify the descriptor in the first file that is added as _dd2.gpkg.\n"
              << "  -o                   (optional, default is jaccard) Specify the objective used for the matching (jaccard,jaccard-hd with two respective weights)."
              << "  -s                   (optional, default is on) Deactivates making use of the Lemmas of optimal solutions w.r.t. Jaccard Index as Preprocessing.\n"
              ...
              << "  -m solution_mode     (optional, default is 3approx) Specify, if the optimal solution (opt) or an approximation (3approx) should be computed.\n"
...
CommandLineOptions parse_command_line(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "-dd1" && i + 1 < argc) {
            options.data_descr1 = argv[++i];
        } else if (arg == "-dd2" && i + 1 < argc) {
            options.data_descr2 = argv[++i];
        } else {
            throw std::invalid_argument("Unknown option: " + arg);
        }
    }
    //make sure data descriptors are in alphabetical order for later processing
    if (options.data_descr1 > options.data_descr2) {
        std::swap(options.data_descr1, options.data_descr2);
    }
