    ...
    for (const auto& e : p.edges()) {
        for (const auto& h : hole_candidates) {
            ...
            for (const auto& e_h : h->edges()) {
                // if e and e_h are equal, this edge does not intersect
                bool e_and_e_h_are_equal= (e.source() == e_h.source() && e.target() == e_h.target()
                    || e.source() == e_h.target() && e.target() == e_h.source());

                if (e_and_e_h_are_equal) {
                    //e does not intersect this hole candidate, we can break
                    break;
                }

                //compute the intersection of the lines
                auto inter = CGAL::intersection(e,e_h);
                if (inter) {
                    if (const Point* ipoint = std::get_if<Point>(&*inter)) {
                        //count as intersection if the intersection point is not near a source or target of e
                        if (CGAL::squared_distance(*ipoint,e.source()) > 1e-2 && CGAL::squared_distance(*ipoint,e.target()) > 1e-2) {
