# AgriMatch: Land-Use Polygon Matching
...
For the solving of linear programs, it can use both `Gurobi`(12.0) and `HiGHS` (1.12.0). Should `Gurobi` be installed on your system, the code will automatically use it, else it will fall back to `HiGHS`. You can manually switch to `HiGHS`by using `-DFORCE_HIGHS=ON` in the cmake command. The code expects `HiGHS`in the subdirectory`external/HiGHS`. You can automatically clone it as a subdirectory via the following command:

```bash 
# while cloning this repo:
git clone --recursive https://github.com/anaumannigg/LandUsePolyMatching.git
# or, after cloning:
...
The project can also be executed as a docker container via the Dockerfile.
For building the docker image, run the following command inside the project directory:
```bash
docker build -t polygon-matching .
```
The docker image runs with the HiGHS solver only.
The image can be executed via:
```bash
docker run -u $(id -u):$(id -g) \
      -v path/to/your/input/directory:/app/input \
      -v path/to/your/output/directory:/app/export \
      tcpolygonmatching [command line options]
```
The code expects the input datasets to be located in a directory with the dataset name inside the directory you specify.
